...
public  class Entity {
    ...
    public Entity(String name) {
        _name = name;
        _key = KeyGen.getKey(16);
        _sorientation = Compass.EAST;
...
//        _orientation = new Vector(getPosition(), target).canonical();
    ...
    public Vector getVector(){
        return new Vector(getPosition(),getPosition().clone().plus(Compass.VECTOR[_sorientation]));
    }
    ...
    public Entity moveForward(int units) {
        return move(Compass.VECTOR[getOrientation()].clone().scalar(units));
    }
    
    public Entity moveUp(int units) {
        return move(Compass.VECTOR[Compass.UP].clone().scalar(units));
    }
    
    public Entity moveDown(int units) {
        return move(Compass.VECTOR[Compass.DOWN].clone().scalar(units));
    }
