...
public class ConsoleAnsi {
    ...
    public ConsoleAnsi(String title) {
        _pconsole = openInternalConsole(getKey());
        _height = 25;
        _width = 80;
    }

    public ConsoleAnsi(String title, int rows, int columns) {

        _pconsole = openExternalConsole(title, rows, columns, 14);

    }

    public ConsoleAnsi(String title, int rows, int columns, int fontsize) {
    ...
    public ConsoleAnsi(String title, String json) {
...
//        System.out.println("Link to external console open on " + _tty);
    ...
    public ConsoleAnsi print(String s) {
        try {
            this.out().append(s);
        } catch (Exception ex) {
    ...
    public ConsoleAnsi println(String s) {
        print(s).print("\n");
        _cursorx = 1;
        _cursory++;
        return this;
    ...
    public ConsoleAnsi captureStdInOut() {
        System.setIn(this.in());
        System.setOut(this.out());
        return this;
    }

    public ConsoleAnsi resetStdInOut() {
    ...
    public String toJson() {
        ...
        res.add("tty", _tty).
                add("text", new JsonArray().add(ConsoleAnsi.getRed(_fgconsole))
                        .add(ConsoleAnsi.getGreen(_fgconsole))
                        .add(ConsoleAnsi.getBlue(_fgconsole))).
                add("background", new JsonArray().add(ConsoleAnsi.getRed(_bgconsole))
                        .add(ConsoleAnsi.getGreen(_bgconsole))
                        .add(ConsoleAnsi.getBlue(_bgconsole)));
    ...
    public ConsoleAnsi resetColors() {
        if (_defbg >= 0) {
     ...
     */
    public ConsoleAnsi setBackground(int color) {
     ...
     */
    public ConsoleAnsi setText(int color) {
    ...
    public ConsoleAnsi clearScreen() {
    ...
    public ConsoleAnsi setCursorXY(int x, int y) {
     ...
     */
    public ConsoleAnsi setCursorOn() {
        String s = "[?25h";
        s = ESC + s;
        print(s);
        return this;
     ...
     */
    public ConsoleAnsi setCursorOff() {
        String s = "[?25l";
        s = ESC + s;
        print(s);
        return this;
     ...
     */
    public ConsoleAnsi doRectangle(int x, int y, int w, int h) {
        for (int i = 0; i < w; i++) {
            for (int j = 0; j < h; j++) {
                setCursorXY(x + i, y + j);
                print(" ");
...
//                }
    ...
    public ConsoleAnsi doRectangleFrame(int x, int y, int w, int h) {
        doRectangle(x, y, w, h);
...
//        print("" + this.windowFrames[0].charAt(3));
    ...
    public ConsoleAnsi doFrame(int x, int y, int x2, int y2) {
        for (int i = y; i <= y2; i++) {
            setCursorXY(x, i);
            print("" + this.windowFrames[0].charAt(5));
            setCursorXY(x2, i);
    ...
    public ConsoleAnsi doGrid(int x, int y, int x2, int y2) {
        for (int i = y; i <= y2; i++) {
            setCursorXY(x, i);
            print("" + this.windowFrames[0].charAt(5));
            setCursorXY(x2, i);
    ...
    public ConsoleAnsi doFrameTitle(String title, int x, int y, int w, int h) {
        doFrame(x, y, x + w - 1, y + h - 1);
        if (title.length() > 0) {
            setCursorXY(x + 1, y);
    ...
    public ConsoleAnsi doProgressBar(int x, int y, int width, double value, double max) {
    ...
    public void printMinibar(int x, int y, int level, int front, int back) {
        ...
        while (level > 0) {
            ...
            if (level >= 8) {
                ...
                setText(front);
                setBackground(back);
                print("" + ConsoleAnsi.windowFrames[2].charAt(level));
            }
            level = level - 8;
        }
    }
    ...
    public void printHMinibar(int x, int y, double level, double maxlevel, int width, int front, int back) {
        ...
        while (ilevel > 0) {
            ...
            if (ilevel >= 8) {
                ...
                setText(front);
                setBackground(back);
                print("" + ConsoleAnsi.windowFrames[3].charAt(ilevel));
            }
            ilevel = ilevel - 8;
        }
    }

//    public ConsoleAnsi printHRuler(int x, int y, int width, int each, int valmax) {
//        int threshold = 0;
...
//        return this;
//    }
    public ConsoleAnsi printHRulerTop(int x, int y, int width, int each, int valmax) {
        int threshold = 0;
...
//                print(String.format("%-4d", (int) ((i * valmax) / width)));
    ...
    public ConsoleAnsi printVRuler(int x, int y, int height, int each, int valmax) {
        int threshold = 0;
    ...
    public ConsoleAnsi printWRuler(int x, int y, int width, int height, int each, int hmax, int vmax) {
...
//
    ...
    public ConsoleAnsi doTextArea(int px, int py, int w, int h, ArrayList<String> lines) {
        resetColors();
...
//        for (int i = (int) (Math.max(0, this.stream.size() - h)); i < this.stream.size() - 1; i++) {
    ...
    public ConsoleAnsi doRadioColor(boolean radio) {
        if (radio) {
